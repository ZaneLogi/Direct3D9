// Basic06_rendering_depth.cpp : Defines the entry point for the application.
//

#include "stdafx.h"
#include "Basic06_rendering_depth.h"
...
void render_frame(void)
{
    ...
    matView = XMMatrixLookAtLH(
        XMLoadFloat3(&XMFLOAT3(0.0f, 0.0f, 15.0f)),   // the camera position
    ...
    XMMATRIX matTranslateA;    // a matrix to store the translation for triangle A
    XMMATRIX matTranslateB;    // a matrix to store the translation for triangle B
    XMMATRIX matRotateY;    // a matrix to store the rotation for each triangle
    static float index = 0.0f; index += 0.05f; // an ever-increasing float value

                                               // build MULTIPLE matrices to translate the model and one to rotate
    matTranslateA = XMMatrixTranslation(0.0f, 0.0f, 2.0f);
    matTranslateB = XMMatrixTranslation(0.0f, 0.0f, -2.0f);
    matRotateY = XMMatrixRotationY(index);    // the front side

                                                // tell Direct3D about each world transform, and then draw another triangle
    d3ddev->SetTransform(D3DTS_WORLD, (const D3DMATRIX*)&(matTranslateA * matRotateY));
    d3ddev->DrawPrimitive(D3DPT_TRIANGLELIST, 0, 1);

    d3ddev->SetTransform(D3DTS_WORLD, (const D3DMATRIX*)&(matTranslateB * matRotateY));
    d3ddev->DrawPrimitive(D3DPT_TRIANGLELIST, 0, 1);
